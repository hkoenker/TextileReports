...
# Test Plot of nets seen/observed
# reg %>% 
#   ggplot() +
#   geom_jitter(aes(x=year, y=percent_nets_seen, color=type_of_survey)) +
#   labs(x="",
#        y="Percent of nets seen",
#        color="") +
#   theme_classic()
# 
# reg %>% 
#   ggplot() +
#   geom_boxplot(aes(x=year, y=percent_nets_seen, color=type_of_survey)) +
#   labs(x="",
#        y="Percent of nets seen",
#        color="",
#        title="Percent of nets observed in MIS/DHS surveys") +
#   theme_classic()
```
...
gh <- dhs_data(countryIds = "GH",
              indicatorIds = "FE_FRTR_W_A15",
              # selectSurveys = "latest", 
              surveyYearStart = 2022,
              breakdown = "subnational") %>% 
  mutate(region = base::iconv (CharacteristicLabel, to="ASCII//TRANSLIT"),
         region = gsub("'", '', region),
         region = tolower(region))
...
# get our related spatial data frame object
spgh <- download_boundaries(surveyId = gh$SurveyId[1], method = "sf")
...
gh <- reg %>% 
  filter(datafile=="GHHR8A") %>% 
  left_join(gh, by = "region") %>% 
  mutate(diffuse = (percent_of_polyester_nets_used-percent_of_polyethylene_nets_used),
        diffuse2 = ifelse(p_value_crudeuse < 0.05, diffuse, NA),
        diff = if_else(p_value_crudeuse > 0.05, -1, diffuse),
    diff = cut(diff, breaks = c(-2, 0, 5, 10, 15, Inf)),
...
mgh <- gh$diff[match(spgh$sdr_subnational_boundaries$DHSREGEN, gh$region)]
spgh$sdr_subnational_boundaries$diff <- mgh
...
mgh <- gh$diffuse2[match(spgh$sdr_subnational_boundaries$DHSREGEN, gh$region)]
spgh$sdr_subnational_boundaries$diffuse2 <- mgh
...
mgh <- gh$aor[match(spgh$sdr_subnational_boundaries$DHSREGEN, gh$region)]
spgh$sdr_subnational_boundaries$aor <- mgh
```

<h2>6. Watch out for in-country variation!</h2>

The below graphs present data on the difference in the percent of nets used the previous night for regions within seven countries where, in at least one survey, both the crude use rate shows a sizeable difference between textiles, and the multivariate regression model indicates textile is significantly associated with use. We can see that large parts of Burkina Faso have no observable difference in use between textiles, while one region leans towards polyethylene nets. For Guinea, the region in the middle of the country shows a slightly higher use of polyester nets. Likewise in Tanzania and Ghana (both 2022), crude differences in use appear in only one or two regions. But remember that net age is not accounted for here - differences in use could be because certain regions of the country have newer nets of one type, and have discontinued use of older nets of the other type.
...
g7 <- ggplot(spgh$sdr_subnational_boundaries) +
  geom_sf(aes(fill = diffuse2), color = "lightgray") +
 theme_void() +
 scale_fill_distiller(palette = "RdBu", direction = 1,
    na.value = "whitesmoke",
...
ggarrange(g1, g21, g3, g4, g5, g6, g7, common.legend = TRUE)
# gn %>% 
#   ggplot(spgn$sdr_subnational_boundaries) +
#   geom_sf(aes(fill = diff))
```

The seven maps below show the results of the multivariate model for each region, controlling for net age, household net supply, wealth quintile, and survey month. Blue regions are where polyethylene nets are less likely to be used (around half as likely in some regions of Ghana, Guinea and  Liberia, compared to polyester), while red regions are where polyethylene nets are more likely to be used (up 2-3 times as likely in some parts of Burkina Faso and Rwanda).
...
ga7 <- ggplot(spgh$sdr_subnational_boundaries) +
  geom_sf(aes(fill = aor), color = "lightgray") +
 theme_void() +
 scale_fill_gradient2(
    low = "lightblue",
...
ggarrange(ga1, ga21, ga3, ga4, ga5, ga6, ga7, common.legend = TRUE)

ggsave(plot=ga6, "images/tz_aor_textile_2022.png")
ggsave(plot=ga7, "images/gh_aor_textile_2022.png")
```
